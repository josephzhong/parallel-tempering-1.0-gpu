...
static __device__ float mix_gauss1_pdf(float x, int k, float* mus,
        float* c1s, float* c2s) {
    float result = 0;
    for (int i = 0; i < k; i++) {
...
// wrapper for above
static __device__ float mix_gauss1_pdf(float x, float* args) {
    int k = (int) args[0];
    float* mus = args + 1;
    float* c1s = args + 1 + k;
...
// template switch
static __device__ float log_mix_gauss1_pdf(float x, int k, float* c1, float* c2,
        float *mus) {
    switch (k) {
    case 2:
...
// wrapper
static __device__ float log_mix_gauss1_pdf(float x, float* args) {
    int k = (int) args[0];
    float* mus = args + 1;
    float* c1s = args + 1 + k;
    float* c2s = args + 1 + 2 * k;
