...
int data_index = tid % (N/lpt);

	for (int i = 0; i < k; i++) {
		if (mus[i] < min || mus[i] > 10) {
			return -FLT_MAX;
		}
	}
	return log_mix_gauss1_mus_pdf<k>(x+data_index*lpt, lpt, c1, c2, mus);
}
...
__device__ float log_mgu_pdf(float* x, int N, float* mus, float c1,
		float c2, float min, float max) {
const int tid = blockDim.x * blockIdx.x + threadIdx.x;
int data_index = (int)( ((int)tid) % ((int)100) );
	for (int i = 0; i < k; i++) {
		if (mus[i] < min || mus[i] > max) {
			return -FLT_MAX;
		}
	}
	return log_mix_gauss1_mus_pdf<k>(x[data_index], c1, c2, mus);
}
*/
template <int k>
__device__ float log_mgu_mu_pdf(float* mus, float* args) {
	...
	float max = args[N + 4];
	float lpt = args[N + 5];
	return log_mgu_pdf<k>(x, N, mus, c1, c2, min, lpt);
}
