...
static __device__ float gauss1_pdf(float x, float c1, float c2, float mean) {
    return c1 * expf(c2 * msquare((x-mean)));
}

// 1D, precomputed constants wrapper
static __device__ float gauss1_pdf(float x, float* args) {
    return gauss1_pdf(x, args[0], args[1], args[2]);
}

// 1D, precomputed constants
static __device__ float log_gauss1_pdf(float x, float c1, float c2, float mean) {
    return logf(c1) + c2 * msquare((x-mean));
}

// 1D, precomputed constants wrapper
static __device__ float log_gauss1_pdf(float x, float* args) {
    return log_gauss1_pdf(x, args[0], args[1], args[2]);
}

// 1D
static __device__ float gauss1_pdf(float x, float mu, float sigma) {
...
// 1D with mean
static __device__ float gauss1_mean_pdf(float x, float mu, float* args) {
