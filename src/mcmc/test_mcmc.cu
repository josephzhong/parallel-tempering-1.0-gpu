...
//	cutStartTimer(hTimer);
	...
	int reduction_size=((int)h_args_p[0])/( (int) h_args_p[ ((int)h_args_p[0]) + 5] );
printf("\ndata_L:%d  index:%d result:%d\n", (int)h_args_p[0], ((int)h_args_p[0]) + 5, (int) h_args_p[ ((int)h_args_p[0]) + 5]);
	...
	printf ("Total FUNC time = %f seconds\n",(double) (tv2.tv_usec - tv1.tv_usec) / 1000000 + (double) (tv2.tv_sec - tv1.tv_sec));
	printf ("NumIterations = %d seconds\n",numIterations);
...
void test_mixture_mus(int M, int nb, int nt, int nbr_data, int lpt) {
	const int k = 4;
...
test_mixture_mus(atoi(argv[1]), atoi(argv[2]), atoi(argv[3]), atoi(argv[4]), atoi(argv[5])); // 8
