...
static __device__ void d_matrix_copy(float* matrix, float* matrix_out, int M, int N) {
    for (int i = 0; i < M * N; i++) {
        matrix_out[i] = matrix[i];
    }
}

static __device__ void d_matrix_add(float* matrix1, float* matrix2, float* matrix_out, int M, int N) {
    for (int i = 0; i < M * N; i++) {
        matrix_out[i] = matrix1[i] + matrix2[i];
    }
}

static __device__ void d_matrix_add(float* matrix1, float value, float* matrix_out, int M, int N) {
    for (int i = 0; i < M * N; i++) {
        matrix_out[i] = matrix1[i] + value;
    }
}

static __device__ void d_matrix_minus(float* matrix1, float value, float* matrix_out, int M, int N) {
    for (int i = 0; i < M * N; i++) {
        matrix_out[i] = matrix1[i] - value;
    }
}

static __device__ void d_matrix_minus(float* matrix1, float* matrix2, float* matrix_out, int M, int N) {
    for (int i = 0; i < M * N; i++) {
        matrix_out[i] = matrix1[i] - matrix2[i];
    }
}

static __device__ void d_matrix_times_diag(float* matrix1, float* diag, float* matrix_out, int M, int N) {
    for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
...
static __device__ void d_matrix_times(float* matrix1, float* matrix2, float* matrix_out, int M1, int N1,
        int M2, int N2) {
    for (int i = 0; i < M1; i++) {
        for (int j = 0; j < N2; j++) {
            float acc = 0;
...
static __device__ void d_vector_add(float* v1, float* v2, float* v_out, int D) {
    for (int i = 0; i < D; i++) {
        v_out[i] = v1[i] + v2[i];
    }
}

static __device__ float* d_vector_get(float* array, int D, int j) {
    return array + j * D;
}

static __device__ void d_vector_set(float* v1, float* v2, int D) {
    for (int i = 0; i < D; i++) {
        v1[i] = v2[i];
    }
}
...
static __device__ void d_vector_swap(float* v1, float* v2, int D) {
    float t;
    for (int i = 0; i < D; i++) {
        t = v1[i];
        v1[i] = v2[i];
...
static __device__ void d_matrix_times(float* matrix, float* matrix_out, float k, int M, int N) {
    for (int i = 0; i < M * N; i++) {
        matrix_out[i] = matrix[i] * k;
    }
}

static __device__ void d_vector_exp(float* v, float* v_out, int D) {
    for (int i = 0; i < D; i++) {
        v_out[i] = expf(v[i]);
    }
}

static __device__ void d_vector_zero(float* v, int D) {
    for (int i = 0; i < D; i++) {
        v[i] = 0;
    }
}
...
static __device__ void d_matrix_sub(float* matrix, float* out, int* r, int M, int N, int j0, int j1) {
...
static __device__ void d_matrix_solve(float* LU, int* piv, float* B, float* X, int MA, int NA, int MB,
        int NB) {
...
static __device__ void d_matrix_solve_L(float* L, float* b, float* x, int D) {
    for (int i = 0; i < D; i++) {
        x[i] = b[i];
    }
    //
...
static __device__ void d_matrix_solve_Lt(float* L, float* b, float* x, int D) {
...
static __device__ void d_matrix_solve_pd(float* L, float* b, float* x, int D) {
    d_matrix_solve_L(L, b, x, D);
    d_matrix_solve_Lt(L, x, x, D);
}

static __device__ void d_matrix_solve_L(float* L, float* B, float* X, int D, int NB) {
...
static __device__ void d_matrix_solve_Lt(float* L, float* B, float* X, int D, int NB) {
    for (int i = 0; i < D * NB; i++) {
        X[i] = B[i];
    }
...
static __device__ void d_matrix_solve_pd(float* L, float* B, float* X, int D, int NB) {
...
static __device__ float d_vector_xty(float* x, float* y, int N) {
    float r = 0;
    for (int i = 0; i < N; i++) {
        r += x[i] * y[i];
    }
